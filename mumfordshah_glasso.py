...
#No longer a projection onto simplex problem
#Now done with ADMM for group LASSO
def project_simplex_glasso(u):
	(ny, nx, k) = u.shape
...
def mumford(fn_in, l=5):
	...
	res_F = project_balls
	#res_G = project_simplex_

	#Need to write a new res_G for group LASSO. Everything else stays the same...
	res_G = project_simplex_glasso
	K = lambda x: grad(x, h)
...
Adds a group LASSO penalty to encourage a sparse number of colors are used.
